...
test_functional_local_hosted:
	...
	docker build --tag matchmaking-test-functional-java -f test/functional/Dockerfile test/functional && \
	docker run --rm -t \
		--env-file $(ENV_PATH) \
		-e GRADLE_USER_HOME=.gradle \
		-u $$(id -u):$$(id -g) \
		-v $$(pwd):/data \
		-w /data matchmaking-test-functional-java bash ./test/functional/test-local-hosted.sh
...
ifeq ($(shell uname), Linux)
	...
	docker run --rm -t \
		...
		-v $$(pwd):/data \
		-w /data matchmaking-test-functional-java bash ./test/functional/test-accelbyte-hosted.sh
